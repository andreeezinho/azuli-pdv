...
export function BarLink({link, text, url, onClick, children, style, className}){
    ...
    return(
        <NavLink to={link} onClick={onClick} className={`${url === location.pathname ? "bg-secondary-dark fill-white text-white" : "bg-[#30384b] fill-none"} hover:bg-secondary-dark w-9 h-9 md:w-full ${className ? "md:h-[35px] my-1" : "md:min-h-[60px] my-4" }  flex items-center justify-center md:justify-normal rounded-md p-1 md:p-0 md:px-5 overflow-hidden group ${style}`}>
            {children}
